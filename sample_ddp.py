...
def main(args):
    ...
    config.model.param["num_classes"] = args.num_classes if not args.is_uncond else 1
    model = create_model(model_config=config.model).to(device)
    ...
    elif args.model_config == "config/DiT-L.yaml":
        vae = AutoencoderKL.from_pretrained(f"stabilityai/sd-vae-ft-ema").to(device)
        scaling_factor = 0.18215
    else:
        raise NotImplementedError(f"pleaes implement vae for config: {args.model_config}")

    assert args.cfg_scale >= 1.0 or args.is_uncond, "In almost all cases, cfg_scale be >= 1.0"
    ...
    for _ in pbar:
        ...
        y = (
            torch.randint(0, args.num_classes, (n,), device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_config", type=str, default="config/DiT-S.yaml", choices=["config/DiT-L.yaml", "config/DiT-S.yaml"])
    ...
    parser.add_argument("--is_uncond", action="store_true")
